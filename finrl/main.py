...
def main() -> int:
    ...
    if options.mode == "train":
        ...
        trade(
            ...
            + 3,  # bug fix: for dqn add dimension of state/observations space =  len(stocks)* len(INDICATORS) + 3+ 3*len(stocks)
            action_dim=len(
                DOW_30_TICKER
            ),  # bug fix: for dqn add dimension of action space = len(stocks)
        )
    else:
